...
- Clinical retrieval: an image/report from another patient should not be treated as an unrelated negative if it shares the same clinically meaningful pathology cluster.
...
- Keep method claims limited to the clean final code: study-level fusion, IDF-Jaccard, Healthy Cluster, clinical curriculum, and auxiliary clinical heads.
